...
func (r *reviewRepo) getData1(ctx context.Context, storeID int64, offset, limit int) ([]*biz.MyReviewInfo, error) {
	...
	for _, hit := range resp.Hits.Hits {
		...
		if err := json.Unmarshal(hit.Source_, tmp); err != nil {
			r.log.Errorf("ListReviewByStoreID unmarshal fail, err:%v", err)
			continue
		}
...
func (r *reviewRepo) getDataBySingleflight(ctx context.Context, key string) ([]byte, error) {
	v, err, shared := g.Do(key, func() (interface{}, error) {
		...
		r.log.Debugf("getDataFromCache(ctx, key) data%s, err:%v\n", data, err)
		if err == nil {
			//缓存命中
			return data, nil
		}
		...
		//查缓存失败了，例如redis down了,直接返回错误，不传到压力
		return nil, err
	})
	r.log.Debugf("singleflight ret :%v, err:%v, shared:%v\n", v, err, shared)
	if err != nil {
		return nil, err
	}
	return v.([]byte), nil
