...
type Msg struct {
	...
	IsDelete bool   `json:"isDdl"`
	Data     []map[string]interface{}
}

// Start kratos程序启动之后干活的
// ctx是kratos框架启动时候传入的ctx，是带有退出取消的
func (jw JobWorker) Start(ctx context.Context) error {
	jw.log.Debug("JobWorker Start")
	//1.从kafka获取Mysql的变更数据
	// 接收消息
	for {
		...
		if err := json.Unmarshal(m.Value, msg); err != nil {
			jw.log.Errorf("Unmarshal message from kafkafailed, err:%v", err)
			continue
		}
		//实际业务场景可能需要在这增加一个步骤：对数据做业务处理
