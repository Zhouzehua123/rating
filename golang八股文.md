...
fmt.Println("len(str):",len(str))  //普通的len处理字符串的长度这里会输出11，

一个汉字是3个长度

fmt.Println("rune:",len([]rune(str)))  //这里就会输出7
...
这个其实可以使⽤ unsafe.Sizeof 计算出⼀个数据类型实例需要占⽤的字节数，空struct{}不占⽤任何
空间。如果想用map实现一个set，就可以让map的键存元素，值存空结构体（因为不占空间），

而map中键又是唯一的，所以可以实现集合。

另一个用途是在一个协程中想传递信号，可以使用channel
...
这个我看过原码，他有count（健值对总数）、hash0（哈希因子，对key生成哈希值）、
...
当本地队列满了的时候，P会把本地队列里**一半**的G放回全局队列，

这是一个双向的平衡机制。

这个机制保证了所有 M 尽可能保持忙碌，最大化利用多核能力。

我还了解，当某个G与他的M发生系统调用，会产生阻塞，

这个时候P队列会立刻与当前M解绑，

去空闲M列表找休眠M并唤醒。当系统调用阻塞结束，

他的M会回到空闲M列表进行休眠，这个G也会重新回到P队列等待。
...
1. 如果锁已被占用，并且锁不处于饥饿模式（这样才会抢锁）。
   ...
   再自旋也是浪费CPU，不如直接阻塞，排队等着。
2. 积累的自旋次数小于最大自旋次数（active_spin=4）。

   如果自旋了4次还没拿到锁，说明这把锁短时间内可能确实放不掉了。

   这时候再自旋下去就变成浪费了，不如阻塞。
3. CPU 核数大于 1。

   如果只有一个CPU核心，一个M在自旋空转，

   就意味着 **所有其他M都无法运行** ，因为CPU被占着。

   这时候自旋纯粹是浪费，持锁的M得不到执行机会，

   锁永远无法释放。所以单核情况下，Go会避免自旋。
4. 有空闲的 P

如果一个M正在自旋（占用着一个P），

同时还有其他空闲的P存在，说明系统还有余力处理其他任务。

让这个M自旋一会儿，既不影响其他P去执行别的G，又有可能快速拿到锁，
...
### 12.如何知道⼀个对象是分配在栈上还是堆上？
 ...
 这就要看他有没有逃逸现象。Go语言通过**逃逸分析**来决定变量分配位置，
