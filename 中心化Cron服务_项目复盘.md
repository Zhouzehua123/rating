...
金融风控平台的在线服务主要处理银行实时提交的请求，聚合下游风险数据并返回评估结果。围绕这些服务，业务和研发还需要持续关注风险情况、下游数据源状态以及服务异常，周期性地完成信息汇总、异常分析和报告整理。

这类工作适合抽象成 Agent 的定时分析任务。以“每天上午汇总上一周期的风险指标和数据源异常，生成分析简报”为例，一次任务需要经过数据查询、异常信息提取、多轮模型与工具调用、报告生成和结果通知。它与同步接口的生命周期不同：用户定义的是一个长期执行计划，单次分析却可能持续数分钟，而且需要在用户离线时继续完成。

项目围绕这类金融风控辅助分析任务，建设统一的 Agent 定时调度能力。Cron 服务管理的是计划、执行记录、异常恢复和结果投递；分析过程中的数据查询与报告生成由执行侧 Agent 及其工具承接。定时分析产出用于辅助排查和研判，在线反欺诈请求仍按原有服务链路处理。
...
- [ ] Job/Run 表最终生产字段与唯一索引。
- [ ] `next_run_at/next_dispatch_at` 和 jitter 是否已按设计上线。
- [ ] CAS 是行锁 + affected rows，还是最终用了 SKIP LOCKED/其他实现。
- [ ] Runner 的真实 IDL 与 Run/Query/Cancel/Finish 名称。
- [ ] Runner 执行账存 Redis 还是 MySQL，fence_token 的最终实现。
- [ ] 哪些 error_code 允许自动重试。
- [ ] DELIVERING 后“不盲目重投”的最终边界。
- [ ] CGI `push_id` 是否使用持久化唯一约束。
- [ ] Checkpoint 做到方案、Demo 还是上线的哪一阶段。
- [ ] 可公开的测试量、故障注入结果与指标，不能用猜测线上成功率。
